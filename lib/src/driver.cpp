...
void process_message(const std::string& data,
    ...
    if (!msg.contains("type") || !msg["type"].is_string()) {
        spdlog::warn(
            "Message received without valid 'type' field, skipping...");
        return;
    }
...
DvlA50Driver::DvlA50Driver(const std::string& ip,
                           int port,
    ...
    socket_.async_read_some(
        ...
        [this, buf](const boost::system::error_code& ec,
            ...
            if (!ec) {
                ...
                process_message(data, velocity_callback_, position_callback_);
                read();
            } else {
